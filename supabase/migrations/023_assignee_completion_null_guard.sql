-- Planly 3.3D — deny completion RPC when assignee_id is NULL or differs from caller.
-- SQL <> returns NULL for NULL operands, which made the prior IF predicate fail open.
...
begin
  ...
  if v_task.visibility <> 'household'
     ...
     or v_task.assignee_id is distinct from auth.uid()
     or not exists (
       select 1 from public.planly_household_members m
       where m.household_id=v_task.household_id
         and m.user_id=auth.uid()
     ) then
    raise exception 'Only the current assignee may complete this task' using errcode='42501';
  end if;

  if v_task.completed is not distinct from p_completed then
    return v_task;
  end if;
