...
load();applyTheme();startPlanlyAuth().then(()=>{if(state?.view==='settings')render()}).catch(()=>{});if(!isStandalone())$('#installHelp').hidden=false;if('serviceWorker'in navigator)Promise.resolve();render();if(googleConnected())syncPendingGoogle().then(()=>render()).catch(()=>{});
})();
