...
function settingsView(){
  ...
  <div class="settingsCard"><h3>Data</h3><button id="exportBtn" class="primary">Export backup</button><button id="importBtn" class="secondaryBtn">Import backup</button><button id="cloudMigrationBackupBtn" class="secondaryBtn">Prepare 3.2 migration backup</button><div class="muted settingsHelp">Use this only for the Planly 3.2 cloud migration. It creates a JSON copy of the exact tasks, projects and settings stored inside this installed Planly app.</div><button id="clearBtn" class="dangerBtn">Clear all data</button></div>
  ...
  $('#cloudMigrationBackupBtn').onclick=exportCloudMigrationBackup;
...
function exportCloudMigrationBackup(){
  const raw=localStorage.getItem(STORE);
  if(!raw){alert('No Planly local data was found on this installed app.');return}
  let parsed;try{parsed=JSON.parse(raw)}catch{alert('Planly local data could not be read. Export stopped.');return}
  const tasks=Array.isArray(parsed.tasks)?parsed.tasks:[],projects=Array.isArray(parsed.projects)?parsed.projects:[];
  if(!tasks.length&&!projects.length){alert('No local tasks or projects were found. Export stopped.');return}
  const payload={kind:'planly-cloud-migration-backup',version:1,exportedAt:new Date().toISOString(),storeKey:STORE,raw,taskCount:tasks.length,projectCount:projects.length};
  const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),a=document.createElement('a');
  a.href=URL.createObjectURL(blob);a.download='planly-cloud-migration-'+localKey(new Date())+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
  showToast('3.2 migration backup prepared · '+tasks.length+' tasks · '+projects.length+' projects');
}
