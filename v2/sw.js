const CACHE='planly-v2-400b01';
const VERSION='planly-v2-sw-400b01';
...
const BUDGET_RUNTIME_URL='./core-budget-v4.0b.js?v=400b02';
...
const REQUIRED=['./index.html',APP_URL,HARDENING_URL,...CORE_URLS,BUDGET_RUNTIME_URL,BUDGET_UI_URL,'./supabase-config.js','./manifest.webmanifest'];
...
async function hardenedAppResponse(request){const cache=await caches.open(CACHE);const [appResponse,hardeningResponse,...coreResponses]=await Promise.all([freshOrCached(cache,APP_URL,request),freshOrCached(cache,HARDENING_URL),...CORE_URLS.map(url=>freshOrCached(cache,url))]);if(!appResponse)return Response.error();let appText=await appResponse.text();const coreTexts=[];for(const response of coreResponses)if(response)coreTexts.push(await response.text());if(coreTexts.length){const closeIndex=appText.lastIndexOf('})();');if(closeIndex<0)return new Response('Planly core injection point missing',{status:500,headers:{'Content-Type':'text/plain','Cache-Control':'no-store'}});appText=appText.slice(0,closeIndex)+'\n'+coreTexts.join('\n')+'\n'+appText.slice(closeIndex)}const hardeningText=hardeningResponse?await hardeningResponse.text():'';return new Response(appText+'\n;'+hardeningText,{status:200,headers:{'Content-Type':'application/javascript; charset=utf-8','Cache-Control':'no-store'}})}
