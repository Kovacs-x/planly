const CACHE='planly-v2-330c05';
const VERSION='planly-v2-sw-330c05';
...
const CORE_BUILD_URL='./core-build-v3.3c.js?v=330c05';
...
const CORE_URLS=[CORE_PROJECTS_URL,CORE_BUILD_URL,CORE_ASSIGNMENT_URL,CORE_PROJECT_PLANNING_URL,CORE_HOUSEHOLD_CALENDAR_URL,CORE_HOUSEHOLD_PLANNING_SAFETY_URL];
...
async function cacheOne(cache,url){
  try{const response=await fetch(url,{cache:'no-store'});if(response?.ok){await cache.put(url,response.clone());return true}}catch{}
  return false;
}
async function networkThenCache(request,cacheKey){
  const cache=await caches.open(CACHE);
  try{const response=await fetch(request,{cache:'no-store'});if(response?.ok)await cache.put(cacheKey,response.clone());return response}
  catch{return (await cache.match(cacheKey))||Response.error()}
}
async function freshOrCached(cache,url,request=null){
  try{const response=await fetch(request||url,{cache:'no-store'});if(response?.ok){await cache.put(url,response.clone());return response}}catch{}
  return cache.match(url);
}
async function hardenedAppResponse(request){
  const cache=await caches.open(CACHE);
  const [appResponse,hardeningResponse,...coreResponses]=await Promise.all([
    freshOrCached(cache,APP_URL,request),
    freshOrCached(cache,HARDENING_URL),
    ...CORE_URLS.map(url=>freshOrCached(cache,url))
  ]);
  if(!appResponse)return Response.error();
  let appText=await appResponse.text();
  const coreTexts=[];
  for(const response of coreResponses)if(response)coreTexts.push(await response.text());
  if(coreTexts.length){
...
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{const cache=await caches.open(CACHE);await Promise.allSettled([...REQUIRED,...OPTIONAL].map(url=>cacheOne(cache,url)));await self.skipWaiting()})());
});
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(key=>key.startsWith('planly-v2-')&&key!==CACHE).map(key=>caches.delete(key)));await self.clients.claim()})());
});
...
self.addEventListener('fetch',event=>{
  ...
  if(url.origin===self.location.origin){
    if(url.pathname.endsWith('/app-v3.2.0.js')){event.respondWith(hardenedAppResponse(event.request));return}
    event.respondWith((async()=>{const cache=await caches.open(CACHE),cached=await cache.match(event.request);if(cached)return cached;return networkThenCache(event.request,event.request)})());return;
  }
