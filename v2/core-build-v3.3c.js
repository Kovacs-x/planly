...
verifyPlanlyOfflineCache=async function(){
  ...
  try{
    const cache=await caches.open('planly-v2-330c02'),assets=['./index.html','./app-v3.2.0.js?v=330c02','./supabase-config.js','./manifest.webmanifest'],missing=[];
...
probePlanlyServiceWorker=async function(){
  ...
  try{
    const res=await planlyWithTimeout(fetch('./__planly_sw_probe__?v=330c02',{cache:'no-store'}),3000,'Service worker probe');
    const text=(await res.text()).trim();
    return {ok:res.ok&&text==='planly-v2-sw-330c02',reason:text||('HTTP '+res.status)};
