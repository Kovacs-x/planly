...
if(new URLSearchParams(location.search).get('planlydiag')==='1'){
  const diag={startedAt:new Date().toISOString(),events:[],max:500};
  const snap=()=>({tab:state?.tab||'',scrollY:Math.round(window.scrollY||0),docH:document.documentElement?.scrollHeight||0,vvH:Math.round(window.visualViewport?.height||0),vvTop:Math.round(window.visualViewport?.offsetTop||0),focus:document.activeElement?.id||document.activeElement?.tagName||''});
  ...
  let lastScrollY=Math.round(window.scrollY||0),scrollTimer=0;
  addEventListener('scroll',()=>{const y=Math.round(window.scrollY||0),delta=y-lastScrollY;lastScrollY=y;clearTimeout(scrollTimer);scrollTimer=setTimeout(()=>log('scroll:settle',{delta}),120)},{passive:true});
