...
function planlyCloudPreviewHtml(){if(!PLANLY_CLOUD_PREVIEW)return '';if(!planlySession?.user)return '<div class="muted settingsHelp">3.2 preview: sign in above before importing your migration backup.</div>';const s=planlyCloudLocalStatus(),pending=readPlanlyPendingWrites().length,label=pending?(pending+' change'+(pending===1?'':'s')+' pending sync'):s.state==='cloud-write-test'?'Cloud write test enabled':s.state==='cloud-loaded'?'Cloud copy loaded · read only':s.state==='complete'?'Migration verified':s.state==='uploading'?'Uploading…':s.state==='verifying'?'Verifying…':s.state==='checking'?'Checking…':s.state==='error'?'Stopped safely':'Not migrated';const counts=(Number.isFinite(s.taskCount)&&Number.isFinite(s.projectCount))?'<br>'+s.taskCount+' tasks · '+s.projectCount+' projects':'';const pendingHelp=pending?'<br><strong>'+pending+' queued change'+(pending===1?'':'s')+' will sync after Planly confirms the server write.</strong>':'';return '<div class="calendarStatusRow"><span class="statusDot '+((!pending&&['complete','cloud-loaded','cloud-write-test'].includes(s.state))?'connected':'offline')+'"></span><strong>3.2 Cloud Preview · '+esc(label)+'</strong></div><div class="muted settingsHelp">'+(planlyCloudReadOnly?'This preview is displaying the verified Supabase copy in read-only bootstrap mode. Changes are not persisted yet.':'Controlled cloud writes are enabled for this preview. Use the disposable conflict-test task below for multi-client testing.')+pendingHelp+counts+'</div><input id="planlyMigrationFile" type="file" accept="application/json,.json" hidden><button id="planlyCloudMigrateBtn" class="secondaryBtn" '+(['checking','uploading','verifying'].includes(s.state)?'disabled':'')+'>'+(s.state==='cloud-loaded'?'Enable controlled write test':s.state==='cloud-write-test'||pending?'Write test enabled':'Verify with migration backup')+'</button>'+((s.state==='cloud-write-test'||pending)?'<button id="planlyConflictTestBtn" class="secondaryBtn">Create / open conflict-test task</button><button id="planlyDeterministicConflictBtn" class="secondaryBtn">Run deterministic conflict test</button>':'')}
