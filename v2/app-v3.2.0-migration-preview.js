...
function planlyCloudPreviewHtml(){if(!PLANLY_CLOUD_PREVIEW)return '';if(!planlySession?.user)return '<div class="muted settingsHelp">3.2 preview: sign in above before importing your migration backup.</div>';const s=planlyCloudLocalStatus(),label=s.state==='cloud-write-test'?'Cloud write test enabled':s.state==='cloud-loaded'?'Cloud copy loaded · read only':s.state==='complete'?'Migration verified':s.state==='uploading'?'Uploading…':s.state==='verifying'?'Verifying…':s.state==='checking'?'Checking…':s.state==='error'?'Stopped safely':'Not migrated';const counts=(Number.isFinite(s.taskCount)&&Number.isFinite(s.projectCount))?'<br>'+s.taskCount+' tasks · '+s.projectCount+' projects':'';return '<div class="calendarStatusRow"><span class="statusDot '+(['complete','cloud-loaded','cloud-write-test'].includes(s.state)?'connected':'offline')+'"></span><strong>3.2 Cloud Preview · '+esc(label)+'</strong></div><div class="muted settingsHelp">'+(planlyCloudReadOnly?'This preview is displaying the verified Supabase copy in read-only bootstrap mode. Changes are not persisted yet.':'Your installed Planly data is isolated by iOS. Select the 3.2 migration backup created from the Home Screen app. The preview validates it before any cloud write.')+''+counts+'</div><input id="planlyMigrationFile" type="file" accept="application/json,.json" hidden><button id="planlyCloudMigrateBtn" class="secondaryBtn" '+(['checking','uploading','verifying'].includes(s.state)?'disabled':'')+'>'+(s.state==='cloud-loaded'?'Enable controlled write test':s.state==='cloud-write-test'?'Write test enabled':'Verify with migration backup')+'</button>'}
