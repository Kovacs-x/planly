...
async function cloudInsertTask(t){const ownerId=planlySession.user.id,row=taskCloudRow(t,ownerId);const {data,error}=await planlySupabase.from('planly_tasks').insert(row).select('client_id,cloud_version').single();if(error)throw error;planlyCloudSyncMeta.tasks.set(String(data.client_id),Number(data.cloud_version));return data}
async function cloudUpdateTask(t){const ownerId=planlySession.user.id,id=String(t.id),version=planlyCloudSyncMeta.tasks.get(id);if(!version)return cloudInsertTask(t);const row=taskCloudRow(t,ownerId);delete row.owner_id;delete row.client_id;const {data,error}=await planlySupabase.from('planly_tasks').update(row).eq('owner_id',ownerId).eq('client_id',id).eq('cloud_version',version).select('client_id,cloud_version').maybeSingle();if(error)throw error;if(!data)throw new Error('Sync conflict: this task changed in another Planly client. Reload before editing it again.');planlyCloudSyncMeta.tasks.set(id,Number(data.cloud_version));return data}
async function cloudDeleteTaskById(id){const ownerId=planlySession.user.id,key=String(id),version=planlyCloudSyncMeta.tasks.get(key);if(!version)throw new Error('Sync conflict: missing cloud version for deleted task.');const {data,error}=await planlySupabase.from('planly_tasks').update({deleted_at:new Date().toISOString(),client_updated_at:Date.now()}).eq('owner_id',ownerId).eq('client_id',key).eq('cloud_version',version).select('client_id,cloud_version').maybeSingle();if(error)throw error;if(!data)throw new Error('Sync conflict: this task changed in another Planly client. Reload before deleting it.');planlyCloudSyncMeta.tasks.set(key,Number(data.cloud_version));return data}
async function cloudInsertProject(p){const ownerId=planlySession.user.id,row=projectCloudRow(p,ownerId);const {data,error}=await planlySupabase.from('planly_projects').insert(row).select('client_id,cloud_version').single();if(error)throw error;planlyCloudSyncMeta.projects.set(String(data.client_id),Number(data.cloud_version));return data}
async function cloudUpdateProject(p){const ownerId=planlySession.user.id,id=String(p.id),version=planlyCloudSyncMeta.projects.get(id);if(!version)return cloudInsertProject(p);const row=projectCloudRow(p,ownerId);delete row.owner_id;delete row.client_id;const {data,error}=await planlySupabase.from('planly_projects').update(row).eq('owner_id',ownerId).eq('client_id',id).eq('cloud_version',version).select('client_id,cloud_version').maybeSingle();if(error)throw error;if(!data)throw new Error('Sync conflict: this project changed in another Planly client. Reload before editing it again.');planlyCloudSyncMeta.projects.set(id,Number(data.cloud_version));return data}
...
async function loadVerifiedCloudPreview(){
  ...
  const previousStatus=planlyCloudLocalStatus();planlyCloudReadOnly=!['cloud-write-test','offline-retry-needed'].includes(previousStatus.state);planlyCloudBootstrapPending=false;setPlanlyCloudLocalStatus({state:planlyCloudReadOnly?'cloud-loaded':'cloud-write-test',taskCount:tasks.length,projectCount:projects.length,loadedAt:new Date().toISOString()});return true;
}
...
async function startPlanlyAuth(){
  ...
  try{await refreshPlanlySession();await loadPlanlyCalendarData();await loadVerifiedCloudPreview()}catch(err){planlyCloudBootstrapPending=false;if(PLANLY_CLOUD_PREVIEW){console.warn('Planly cloud bootstrap stopped safely',err);setPlanlyCloudLocalStatus({state:isOfflineCloudError(err)?'offline-retry-needed':'error',error:String(err?.message||err)})}}
