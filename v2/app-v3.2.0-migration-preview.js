...
function queueCloudWrite(work,success='Synced to Planly Cloud'){if(!PLANLY_CLOUD_PREVIEW||planlyCloudReadOnly)return Promise.resolve();planlyCloudWriteQueue=planlyCloudWriteQueue.then(work).then(v=>{showToast(success);return v}).catch(err=>{showToast('Cloud sync stopped · reload before more edits');alert(err?.message||'Cloud sync failed.');throw err});return planlyCloudWriteQueue}
...
async function loadVerifiedCloudPreview(){
  if(!PLANLY_CLOUD_PREVIEW||!planlySession?.user||!initPlanlySupabase())return false;
...
async function startPlanlyAuth(){
  ...
  try{await refreshPlanlySession();await loadPlanlyCalendarData();await loadVerifiedCloudPreview()}catch(err){if(PLANLY_CLOUD_PREVIEW)console.warn('Planly cloud bootstrap stopped safely',err)}
...
load();applyTheme();startPlanlyAuth().then(()=>render()).catch(()=>{});if(!isStandalone())$('#installHelp').hidden=false;if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js').catch(()=>{});render();if(googleConnected())syncPendingGoogle().then(()=>render()).catch(()=>{});
})();
