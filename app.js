...
function todayView(){
  ...
  const rest=sortTasks(visibleTasks(todayAll.filter(t=>!t.pinned)));
...
function monthView(){const first=monthStart(state.monthAnchor),d=parseKey(first),year=d.getFullYear(),month=d.getMonth();setHeader(new Intl.DateTimeFormat(undefined,{month:'long'}).format(d),String(year));const offset=(d.getDay()+6)%7;const days=new Date(year,month+1,0).getDate();let cal='<div class="calendar">'+['M','T','W','T','F','S','S'].map(x=>`<div class="dow">${x}</div>`).join('');for(let i=0;i<offset;i++)cal+='<div></div>';for(let day=1;day<=days;day++){const k=localKey(new Date(year,month,day,12)),c=state.tasks.filter(t=>t.date===k).length;cal+=`<button class="day ${c?'has':''} ${state.selectedDate===k?'selected':''}" data-date="${k}">${day}${c?`<span style="font-size:9px;margin-left:2px">•</span>`:''}</button>`}cal+='</div>';const arr=visibleTasks(state.tasks.filter(t=>t.date===state.selectedDate));$('#view').innerHTML=`<div class="toolbar"><button id="prevMonth">‹</button><button id="todayMonth">Today</button><button id="nextMonth">›</button></div>${cal}<section class="section"><div class="sectionHead"><h2>${fmt(state.selectedDate,{weekday:'long',day:'numeric',month:'long'})}</h2></div>${arr.length?arr.map(taskHtml).join(''):`<div class="empty">No tasks for this date.</div>`}</section>`;$('#prevMonth').onclick=()=>{state.monthAnchor=shiftMonth(first,-1);state.selectedDate=state.monthAnchor;render()};$('#nextMonth').onclick=()=>{state.monthAnchor=shiftMonth(first,1);state.selectedDate=state.monthAnchor;render()};$('#todayMonth').onclick=()=>{state.monthAnchor=localKey(new Date());state.selectedDate=localKey(new Date());render()};$$('.day[data-date]').forEach(b=>b.onclick=()=>{state.selectedDate=b.dataset.date;render()})}
...
function render(){ $$('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.tab===state.tab)); $('#addBtn').style.display=state.tab==='settings'?'none':'block'; if(state.tab==='today')todayView();else if(state.tab==='week')weekView();else if(state.tab==='month')monthView();else if(state.tab==='inbox')inboxView();else settingsView(); bindTaskActions() }
