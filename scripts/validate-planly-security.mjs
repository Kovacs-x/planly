...
for(const table of ['planly_budget_scopes','planly_budget_categories','planly_budget_targets','planly_budget_entries']){
  if(!new RegExp(`create\\s+table\\s+(?:public\\.)?${table}[\\s\\S]{0,2500}cloud_version\\s+bigint`, 'i').test(migrations)){
    fail(`Budget table lacks cloud_version concurrency field: ${table}`);
  }
}
