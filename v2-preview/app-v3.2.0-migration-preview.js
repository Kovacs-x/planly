...
function planlyBulkSafetyCoverageAudit(){
  ...
  const checks=[
    {name:'restore stages durable journal before local replacement',ok:restoreText.indexOf('stagePlanlyBulkJournal')>=0&&restoreText.indexOf('stagePlanlyBulkJournal')<restoreText.indexOf('state.tasks=')},
    ...
    {name:'clear uses tombstones not hard deletes',ok:!restoreText.includes('.delete(')&&!clearText.includes('.delete(')&&projectDeleteText.includes('deleted_at')&&projectDeleteText.includes(".eq('cloud_version',version)")},
...
async function runPlanlyReleaseGate(btn){
  if(!PLANLY_CLOUD_PREVIEW||planlyCloudReadOnly||!planlySession?.user)throw new Error('Enable controlled cloud writes before running the release gate.');
  if(!navigator.onLine)throw new Error('The 3.2 release gate needs an online connection.');
  if(readPlanlyPendingWrites().length||readPlanlyConflicts().length)throw new Error('Resolve or sync existing pending changes before running the release gate.');
  const steps=[];if(btn){btn.disabled=true;btn.textContent='Running 3.2 release gate…'}
  try{
    const bulk=planlyBulkSafetyCoverageAudit();if(!bulk.passed)throw new Error('Bulk safety audit failed: '+bulk.checks.filter(x=>!x.ok).map(x=>x.name).join(', '));steps.push('bulk-safety-audit');
    const backup=planlyBackupValidatorAudit();if(!backup.passed)throw new Error('Backup validator audit failed: '+backup.checks.filter(x=>!x.ok).map(x=>x.name).join(', '));steps.push('backup-v2-validator');
    const recovery=planlyRecoveryCoverageAudit();if(!recovery.passed)throw new Error('Recovery coverage audit failed: '+recovery.checks.filter(x=>!x.ok).map(x=>x.name).join(', '));steps.push('recovery-static-audit');
    const runtime=planlyRuntimeBindingAudit();if(!runtime.passed)throw new Error('Runtime binding audit failed: '+runtime.checks.filter(x=>!x.ok).map(x=>x.name).join(', '));steps.push('runtime-bindings');
    const integrity=await runPlanlySyncIntegritySuite(null);if(!integrity?.passed)throw new Error('The consolidated sync integrity suite did not pass.');steps.push('sync-integrity-suite');
    await reconcilePlanlyCloud({render:false,replay:false});
    ...
    const cloud=await planlyReleaseCloudReadAudit();steps.push('cloud-read-consistency');
    setPlanlyCloudLocalStatus({releaseGate:'passed',releaseGateAt:new Date().toISOString(),releaseGateSteps:steps,releaseGateCloudCounts:cloud,releaseGateOfflineShell:'pending-final-integration'});
    render();showToast('3.2 release gate passed');
    return {passed:true,steps,bulk,backup,recovery,runtime,cloud,offlineShell:'pending-final-integration'};
  }catch(err){
    setPlanlyCloudLocalStatus({releaseGate:'failed',releaseGateAt:new Date().toISOString(),releaseGateSteps:steps,releaseGateError:String(err?.message||err),releaseGateOfflineShell:'pending-final-integration'});
    render();throw err;
  }finally{if(btn){btn.disabled=false;btn.textContent='Run 3.2 release gate'}}
}
...
document.addEventListener('click',e=>{const b=e.target.closest('#planlyReleaseGateBtn');if(!b)return;runPlanlyReleaseGate(b).catch(err=>{showToast('3.2 release gate failed');alert(err?.message||'3.2 release gate failed.')})});
