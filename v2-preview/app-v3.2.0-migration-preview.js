...
function stagePlanlyPendingWrite(kind,action,item,baseVersion){const id=kind==='preference'?'preferences':String(item?.id||item||'');if(!id)return;const current=readPlanlyPendingWrites(),existing=current.find(x=>x.kind===kind&&x.id===id),list=current.filter(x=>!(x.kind===kind&&x.id===id)),stableBase=existing?Number(existing.baseVersion||0):Number(baseVersion||0);list.push({kind,action,id,data:action==='delete'?null:item,baseVersion:stableBase,stagedAt:existing?.stagedAt||new Date().toISOString(),lastEditedAt:new Date().toISOString()});writePlanlyPendingWrites(list);persistPlanlyCloudCache()}
