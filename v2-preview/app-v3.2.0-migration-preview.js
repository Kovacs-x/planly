...
function planlyReleaseGateAudit(){
  const source=document.documentElement.outerHTML,fn=n=>Function.prototype.toString.call(n);
  const checks=[
    ...
    {name:'recurrence generation guarded',ok:fn(completeTaskWithUndo).includes('createNextOccurrence')&&fn(createNextOccurrence).includes('recurrence')},
    ...
    {name:'bulk restore stages before mutation',ok:fn(restorePlanlyBackupToCloud).indexOf('stagePlanlyBulkJournal')<fn(restorePlanlyBackupToCloud).indexOf('state.tasks=backup.tasks')},
    {name:'bulk clear stages before mutation',ok:fn(clearAllPlanlyCloudData).indexOf('stagePlanlyBulkJournal')<fn(clearAllPlanlyCloudData).indexOf('state.tasks=[]')},
...
async function runPlanlySyncIntegritySuite(btn){
  ...
  try{
    ...
    setPlanlyCloudLocalStatus({integritySuite:'passed',integritySuiteAt:new Date().toISOString(),integritySuiteSteps:steps,mutationAuditCount:audit.checks.length,recoveryAuditCount:recoveryCoverage.checks.length,releaseGateCount:releaseGate.checks.length,releaseGate:'passed'});
    removeLocalTestRows();persistPlanlyCloudCache();render();showToast('3.2 release gate passed');
    return {passed:true,steps,audit,recoveryCoverage,releaseGate};
  }catch(err){
