...
const PLANLY_OFFLINE_CACHE='planly-preview-v3-2-p48';
const PLANLY_SW_PROBE='planly-preview-sw-p48';
...
async function verifyPlanlyOfflineCache(){
  ...
  try{
    const cache=await caches.open(PLANLY_OFFLINE_CACHE),assets=['./index.html','./app-v3.2.0-migration-preview.js?v=320p48','./supabase-config.js','./manifest.webmanifest'],missing=[];
...
async function probePlanlyServiceWorker(){
  ...
  try{
    const res=await planlyWithTimeout(fetch('./__planly_sw_probe__?v=p48',{cache:'no-store'}),3000,'Service worker probe');
